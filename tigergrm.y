...
/* TIGER PROGRAMS: Un programa Tiger no es mas q una expresion */
...
exp :     NRO						{ IntExp($1, P()) }
	...
	| FOR id DOSPIG exp TO exp DO exp		{ ForExp({var=$2, escape=ref false,
							  lo=$4, hi=$6, body=$8}, P()) }
	| LET decs IN END				{ LetExp({decs=$2, body=UnitExp(P())}, P()) }
	| LET decs IN exp END				{ LetExp({decs=$2, body=$4}, P()) }
	| LET decs IN exp PCOMA explist END		{ LetExp({decs=$2, body=SeqExp($4::$6, P())}, P()) }
	| l_value CI exp CD OF exp			{ ArrayExp({typ=nombre $1, size=$3, init=$6}, P()) }
...
/* Una declaracion es una secuencia de declaraciones
   de tipos, variables y funciones */
...
fundec : FUNCTION id PI tyflds PD IGUAL exp		{ FunctionDec[({name=$2, params=$4, result=NONE, body=$7}, P())] }
	| FUNCTION id PI tyflds PD DOSP id IGUAL exp	{ FunctionDec[({name=$2, params=$4, result=SOME $7, body=$9}, P())] }
	;
...
/* L-Values:
   Un L-Value puede ser, una variable, parametros de procedimientos,
   campos de records y elementos de un arreglo */
