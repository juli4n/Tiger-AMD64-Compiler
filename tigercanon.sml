...
fun basicBlocks stms = 
	let	val done = tigertemp.newlabel()
		...
		| blocks(stms, blist) = blocks(LABEL(tigertemp.newlabel())::stms, blist)
	in (blocks(stms,nil), done) end

fun enterblock(b as (LABEL s :: _), table) = tabRInserta(labelastring s, b, table)
...
fun trace(table,b as (LABEL lab :: _),rest) = 
	let	val table = tabRInserta(labelastring lab, nil, table)
	in	case splitlast b of
		(most,JUMP(NAME lab, _)) =>
			(case tabBusca(labelastring lab, table) of
			SOME(b' as _::_) => most @ trace(table, b', rest)
			| _ => b @ getnext(table,rest))
		| (most,CJUMP(opr,x,y,t,f)) =>
			(case (tabBusca(labelastring t,table), tabBusca(labelastring f,table)) of
...
val canonize = traceSchedule  o basicBlocks  o linearize   

val canonfrags = ref []

fun canoFrags l = 
	let
		fun f (tigerframe.PROC (body,frame)) = 
				let
(*					val _ = print ("\n\t\t\t" ^ labelastring (name frame) ^ "\n") *)
					val l = canonize body
(*					val _ =  List.app (print o tigerit.tree) l  *)
				in  canonfrags := !canonfrags @ [CPROC (l,frame)] end 
			| f (tigerframe.STRING (label,str)) = canonfrags := !canonfrags @ [CSTRING (label,str)]
	in 
