...
fun tree s0 =
	let	fun say s = s
		...
		| exp(TEMP t, d) = indent(d)^say("TEMP ")^say(tempastring t)
		| exp(ESEQ(s,e),d) = indent(d)^sayln("ESEQ(")^stm(s,d+1)^sayln(",")^
			exp(e,d+1)^say(")")
		| exp(NAME lab, d) = indent(d)^say("NAME ")^say(labelastring lab)
